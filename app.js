// ===== CONFIGURAÇÕES =====
const WHATS_NUMERO   = "5511945414667";                  // 55 + DDD + número (somente dígitos)
const MENSAGEM_PADRAO = "Olá! Vim do site Sidrica Essências e quero fazer um pedido.";
const INSTAGRAM_URL   = "https://instagram.com/gustavo_.diaz"; // seu @

// Monta link do WhatsApp (wa.me)
const linkZap = (msg = MENSAGEM_PADRAO) =>
  `https://wa.me/${WHATS_NUMERO}?text=${encodeURIComponent(msg)}`;

// Aplica nos botões e ícones de WhatsApp
function wireWhats(){
  // Botão flutuante e ícone do rodapé
  const list = [document.getElementById('zap-float'), document.getElementById('foot-wa')];
  list.forEach(el => { if (el) el.href = linkZap(); });

  // Botões de comprar por produto (mensagem com nome)
  document.querySelectorAll('.buy[data-prod]').forEach(a => {
    const nome = a.getAttribute('data-prod');
    a.addEventListener('click', (e) => {
      e.preventDefault();
      window.location.href = linkZap(`Olá! Tenho interesse em "${nome}".`);
    });
  });
}

// Instagram (botão “Seguir” + rodapé + handle)
function wireInstagram(){
  const profileBtn = document.getElementById('btn-insta');
  const footIg = document.getElementById('foot-ig');
  const handle = document.getElementById('insta-handle');
  if (profileBtn) profileBtn.href = INSTAGRAM_URL;
  if (footIg) footIg.href = INSTAGRAM_URL;
  if (handle) handle.textContent = '@' + INSTAGRAM_URL.replace(/^https?:\/\/(www\.)?instagram\.com\//,'').replace(/\/$/,'');
}

// Ano no rodapé
function setYear(){
  const el = document.getElementById('ano');
  if (el) el.textContent = new Date().getFullYear();
}

// Init
wireWhats();
wireInstagram();
setYear();
